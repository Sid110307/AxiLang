...
/*
	...
	if (protocolEnd != std::string::npos)
	{
		...
		std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Host: " << host
				  << std::endl;
		std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Path: " << path
				  << std::endl;
	} else
	{
		std::cerr << "[\033[1;31mERROR\033[0m]: Invalid URL \"" << cleanText(url) << "\"." << std::endl;
		exit(EXIT_FAILURE);
	}

	std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Initializing socket."
			  << std::endl;
	...
	std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Sending request." << std::endl;

	std::string request = "GET " + path + " HTTP/1.1\r\nHost: " + host + "\r\nAccept: * / *\r\nConnection: close\r\n\r\n";
	asio::write(socket, asio::buffer(request));

	std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Sent: \"" << cleanText(request)
			  << "\"." << std::endl;
	std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Reading response."
			  << std::endl;
	...
	std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Received: \""
			  << cleanText(httpResponse) << "\"." << std::endl;

	if (httpResponse.find("301 Moved Permanently") != std::string::npos)
	{
		std::string newUrl;

		while (std::getline(responseStream, httpResponse))
		{
			if (httpResponse.find("Location: ") != std::string::npos)
			{
				newUrl = httpResponse.substr(10);
				break;
			}
		}

		std::cout << "[\033[1;30mDEBUG\033[0m]: " << std::string(redirectLevel * 2, ' ') << "Redirecting to \""
				  << cleanText(newUrl) << "\"." << std::endl;
		return downloadFile(newUrl, redirectLevel + 1);
	}
